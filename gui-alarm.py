...
def set_alarm():
    ...
    alarm_hour = get_hour.get()
    alarm_min = get_min.get()
    ...
    alarm_time = datetime.datetime.combine(now.date(), datetime.time(int(alarm_hour), int(alarm_min), 0))
    print(alarm_time)
...
get_hour = tk.Entry(root)
get_hour.place(x=70, y=100)
get_hour.insert(INSERT, "hour")

get_min = tk.Entry(root)
get_min.place(x=260, y=100)
get_min.insert(INSERT, "min")

B = tk.Button(root, text="Set The Alarm", command=set_alarm)
B.pack()
